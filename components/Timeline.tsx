        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <div className={`flex-1 w-full md:w-1/2 pl-12 md:pl-0 ${index % 2 === 0 ? 'md:pl-24 md:pr-12' : 'md:px-12'}`}>
              <div
                className={`bg-white p-6 rounded-xl border border-slate-200 shadow-md hover:border-accent-blue/50 transition-all duration-300 hover:-translate-y-1 group relative overflow-hidden ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'
                  }`}
              >
                ...
                <h3 className={`relative z-10 text-slate-900 font-bold text-xl mb-2 ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
                  {item.title}
                </h3>
                {item.image && (
                ...
                {item.description && (
                  <p className={`relative z-10 text-slate-600 text-sm leading-relaxed group-hover:text-slate-800 transition-colors ${index % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
                    {item.description}
                  </p>
                )}
              </div>
            ...
            {/* Dot */}
            <div className="absolute left-4 md:left-1/2 transform -translate-x-1/2 w-4 h-4 bg-white border-2 border-accent-blue rounded-full z-10 shadow-[0_0_10px_rgba(59,130,246,0.5)] group-hover:shadow-[0_0_20px_rgba(59,130,246,0.8)] transition-shadow duration-300">
